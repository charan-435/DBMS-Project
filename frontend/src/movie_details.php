...
foreach ($directorStats as $d) {
  if ($d['director'] === $movie['director_name']) {
    $thisDir = $d;
    break;
  }
}
...
foreach ($dirCareer as $dc) {
  if ($dc['yr'] < $movie['release_year'])
    $filmsBefore += 1;
  else if ($dc['yr'] == $movie['release_year'])
    break;
}
...
function fmtRev($n)
{
  ...
  if ($n >= 1e7)
    return '₹' . number_format($n / 1e7, 1) . 'Cr';
  if ($n >= 1e5)
    return '₹' . number_format($n / 1e5, 1) . 'L';
  return '₹' . number_format($n);
}

if ($movie['revenue'] > 5e9) {
  $perfLabel = '🔥 Global Blockbuster';
  $perfCls = '#f5c518';
} elseif ($movie['revenue'] > 1e9) {
  $perfLabel = '💎 Commercial Success';
  $perfCls = '#5cd6b6';
} else {
  $perfLabel = '📈 Steady Performer';
  $perfCls = '#7eafe8';
}
...
<head>
  ...
  <style>
    ...
    .rating-fill {
      height: 100%;
      background: linear-gradient(90deg, #f5c518, #ffd000);
      border-radius: 4px;
    }
        ...
        <div>
          ...
          <div class="insights-row">
            <div class="insight-card-mini">
              ...
              <div class="mini-desc">This film marks the <?= $nthFilm ?>-th theatrical release for director
                <?= htmlspecialchars($movie['director_name']) ?> in our dataset.</div>
            </div>
            ...
            <?php if ($synergy && $synergy['films_together'] > 1): ?>
              <div class="insight-card-mini" style="border-color: var(--accent-green);">
                ...
                <div class="mini-val"><?= $synergy['films_together'] ?><span style="font-size: 0.8rem; opacity: 0.6;">
                    collabs</span></div>
                <div class="mini-desc">This duo (<?= htmlspecialchars($movie['director_name']) ?> &amp;
                  <?= htmlspecialchars($movie['cast'][0]['name']) ?>) averages ★
                  <?= number_format($synergy['avg_rating'], 1) ?> across their projects.</div>
              </div>
            <?php endif; ?>
          </div>

          <p style="color:var(--text-secondary); line-height:1.7; max-width:600px; font-size:0.86rem;">
            Directed by <a href="director_details.php?id=<?= $movie['director_id'] ?>"
              style="color:var(--accent-primary); font-weight:700; text-decoration: underline;"><?= htmlspecialchars($movie['director_name']) ?></a>,
            this <?= htmlspecialchars($movie['genre_name']) ?> production marks the
            **<?= $nthFilm ?><?= $nthFilm == 1 ? 'st' : ($nthFilm == 2 ? 'nd' : ($nthFilm == 3 ? 'rd' : 'th')) ?>** film
            in the director's storied career.
            With <?= count($movie['cast']) ?> credited cast members and a box office revenue of
            <?= fmtRev($movie['revenue']) ?>,
            <?php if ($genreAvgRev > 0): ?>
              the film performed **<?= round(($movie['revenue'] / $genreAvgRev) * 100) ?>%** relative to the genre
              average.
            <?php endif; ?>
          </p>
        </div>
          ...
          <div class="info-card">
            ...
            <div class="cast-chips">
              <?php foreach ($movie['cast'] as $actor): ?>
                <a href="actor_details.php?id=<?= $actor['actor_id'] ?>" class="cast-chip"
                  title="Appeared in <?= $actor['movie_count'] ?> films">
                  <?= htmlspecialchars($actor['name']) ?>
                  <span style="font-size: 0.6rem; opacity: 0.7; margin-left: 4px;">(<?= $actor['movie_count'] ?>
                    films)</span>
                </a>
              <?php endforeach; ?>
          ...
          <?php if (count($directorFilms) > 1): ?>
            <div class="info-card">
              ...
              <?php foreach ($directorFilms as $df): ?>
                <?php if ($df['title'] === $movie['title'])
                  continue; ?>
                <div class="film-row">
                  <a href="movie_details.php?id=<?= $df['movie_id'] ?>"
                    class="film-row-title"><?= htmlspecialchars($df['title']) ?></a>
        ...
        <div>
          <div class="info-card">
            ...
            <div class="stat-row">
              ...
              <span class="stat-val"
                style="font-size:0.72rem; color:var(--text-secondary);">₹<?= number_format($movie['revenue']) ?></span>
            </div>
          </div>
          ...
          <div class="info-card">
            ...
            <div class="stat-row">
              ...
              <span class="stat-val"><a href="movies.php?q=<?= urlencode($movie['director_name']) ?>"
                  style="color:var(--accent-primary); text-decoration:none;"><?= htmlspecialchars($movie['director_name']) ?></a></span>
            </div>
            <div class="stat-row">
          ...
          <div class="info-card">
            ...
            <a href="movies.php?lang=<?= urlencode($movie['language']) ?>"
              style="display:block; padding:0.6rem 0; font-size:0.82rem; color:var(--accent-green); text-decoration:none; border-bottom:1px solid var(--border-color);">🌍
              More <?= strtoupper($movie['language']) ?> films →</a>
            <a href="movies.php?min_year=<?= $movie['release_year'] - 2 ?>&max_year=<?= $movie['release_year'] + 2 ?>"
              style="display:block; padding:0.6rem 0; font-size:0.82rem; color:var(--text-secondary); text-decoration:none;">📅
              Films from <?= $movie['release_year'] - 2 ?>–<?= $movie['release_year'] + 2 ?> →</a>
          </div>
        </div>
      </div>
